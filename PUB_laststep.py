# %%
import models.microstruc_laststep_GINOT as microstruc_laststep_GINOT
import importlib
...
# %%
importlib.reload(microstruc_laststep_GINOT)
LoadModel = microstruc_laststep_GINOT.LoadModel
...
def predict(data_loader, ginot):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader, desc="Predicting"):
            ...
            # pred = ginot(xyt, pc)
            pred = torch.zeros_like(y_true_batch)
...
# %%
index = 3  # sort_idx_with_cells[1] #3
...
xyt = torch.from_numpy(
    np.stack((verts_test, verts_test), axis=0)).to(device)
mask = pc_test != configs.PADDING_VALUE
pc_no_padded = pc_test[mask].reshape(-1, 2)
# Add noise to pc_no_padded
noise = torch.randn_like(pc_no_padded) * 0
pc_no_padded = pc_no_padded + noise

# pc_no_padded_reverse = pc_no_padded.flip(dims=[0])
pc_no_padded_reverse = pc_no_padded[torch.randperm(pc_no_padded.size(0))]
# Combine the original and reversed point clouds into a single tensor
pc_stacked = torch.cat((pc_no_padded.unsqueeze(
    0), pc_no_padded_reverse.unsqueeze(0)), dim=0)
S_test = ginot(xyt, pc_stacked.to(device), order_invariant=False)
S_test = su_inverse(S_test)
S_test = S_test.cpu().detach().numpy()
True_s_test_4 = [True_s_test, True_s_test]
...
# pc_no_padded_shuff = pc_no_padded[torch.randperm(pc_no_padded.size(0))]

# pc = pc_no_padded_shuff[None, :].to(device)
# pred_np_no_padded_shuff = ginot(xyt, pc)
# pred_np_no_padded_shuff = su_inverse(pred_np_no_padded_shuff)
# pred_np_no_padded_shuff = pred_np_no_padded_shuff.cpu().detach().numpy()


# # pc_reduced = pc_no_padded[torch.randperm(pc_no_padded.size(0))]
# # pc_reduced = pc_reduced[:(176-44*0)]
...
# S_test = [pred_np_no_padded_shuff[0], pred_np_pad[0]]
# l2erros_pc = cal_l2_error(S_test, True_s_test_4)
# print(l2erros_pc)
