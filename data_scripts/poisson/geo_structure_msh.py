...
def run_sim():
    ...
    cent = gmsh.model.geo.addPoint(0, 0, 0, lc)
    for xy in xy_bound:
    ...
    # Define radial lines
    radial_lines = []
    for p in point_tags:  # Exclude last duplicated point
        radial_lines.append(gmsh.model.geo.addLine(cent, p))

    surfaces = []
    for i in range(len(line_tags)):
        curve_loop = gmsh.model.geo.addCurveLoop(
            [radial_lines[i], line_tags[i], -radial_lines[(i+1) % len(line_tags)]])
        surf = gmsh.model.geo.addPlaneSurface(
            [curve_loop])
    ...
    surface_group = gmsh.model.addPhysicalGroup(
        2, surfaces, phy_tag, name='Interior')
    phy_tag += 1
    ...
    for line in radial_lines:
        gmsh.model.mesh.setTransfiniteCurve(line, 20)
    for surface in surfaces:
        gmsh.model.mesh.setTransfiniteSurface(surface, arrangement="left")
...
def run_all():
    ...
    while len(cells_all) < 6001:
        ...
        print("the sample has {} nodes".format(len(nodes)))
        print(
    ...
    with open(filebase+'poisson_geo_struc_msh.pkl', 'wb') as f:
