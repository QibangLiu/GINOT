...
# index = sort_idx_with_cells[0]  # 1843
# print(
...
# index = sort_idx_with_cells[int(len(sort_idx_with_cells)*0.5)-2]  # 1674
# print(
...
# index = sort_idx_with_cells[int(len(sort_idx_with_cells)*0.95)-5]  # 177
# print(
...
noise = torch.randn_like(pc_no_padded) * 1e-8
pc_no_padded = pc_no_padded + noise
pc_test[mask] = pc_no_padded.reshape(-1)

# # no padding
pc = pc_no_padded[None, :].to(device)
pred_no_pad = ginot(xyt, pc, order_invariant=order_invariant)
pred_no_pad = su_inverse(pred_no_pad)
pred_no_pad = pred_no_pad.cpu().detach().numpy()

# padding
pc = pc_test[None, :].to(device)
pred_pad = ginot(xyt, pc, order_invariant=order_invariant)
pred_pad = su_inverse(pred_pad)
pred_pad = pred_pad.cpu().detach().numpy()

# shuffled reduced
pc_reduced = pc_no_padded[torch.randperm(pc_no_padded.size(0))]
pc_reduced = pc_reduced[:(220-44)]
pc = pc_reduced[None, :].to(device)
pred_np_reduced = ginot(xyt, pc, order_invariant=order_invariant)
...
pred_np_shuffled = ginot(xyt, pc, order_invariant=order_invariant)
...
S_test = [pred_no_pad[0], pred_np_reduced[0],
          pred_pad[0], pred_np_shuffled[0]]
l2erros_pc = cal_l2_error(S_test, True_s_test_4)
print("no padding,shuffled reduced, padded, shuffled padded:")
print(l2erros_pc)
